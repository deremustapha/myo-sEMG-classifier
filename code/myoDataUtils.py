...
def get_male_data(total_male_subject, total_exercise, abs_path):
...
def get_female_data(total_female_subject, total_exercise, abs_path):
    
    full_buffer = [] 
    
    for subject in range(total_female_subject):
        ...
        for exercise in range(total_exercise):
            ...
            per_subject = abs_path+"/Female" + str(subject) + "/training0" + "/classe_" + str(exercise) + ".dat"
            read_data = np.fromfile(per_subject, dtype=np.int16)
            data_array = np.array(read_data, dtype=np.float32)
...
def get_data(abs_path, n_male, n_female, n_exercise):
    ...
    male_X, male_y = get_male_data(total_male_subject, total_exercise, abs_path)
    female_X, female_y = get_male_data(total_female_subject, total_exercise, abs_path)
    
    train_feature = np.row_stack((male_X, female_X))
    train_label = np.row_stack((male_y, female_y))
